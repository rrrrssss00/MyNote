...
#读取指定的矢量图层，并将其导入到Oracle数据库的指定表中（按批次版本）
def importShpBulk(importFileName:str,tableNameToImport:str):
    # region 连接数据库
    ...
    for i in range(0,fieldCount):
        ...
        fieldNameLst.append(tmpFieldName)
        fieldNameStr += tmpFieldName+","
        fieldValStr += ":"+tmpFieldName+","
    fieldNameStr += "SHAPE)"
    fieldValStr += "sdo_geometry(:SHAPE,4610))"
    #endregion
    ...
    for i in range(0,featureCount):
        ...
        for j in range(0,fieldCount):
            tmpValLst[fieldNameLst[j]] = tmpfea.GetFieldAsString(j)
        tmpGeo = tmpfea.GetGeometryRef()                                #type:ogr.Geometry
        wkt = tmpGeo.ExportToWkt()

        tmpValLst["SHAPE"] = wkt

        valLst.append(tmpValLst)

    inputSize = {}
    inputSize["SHAPE"] = cx_Oracle.CLOB
    cur.setinputsizes(**inputSize)
    cur.bindarraysize = len(valLst)
    #print ("insert into "+tableNameToImport+ " "+fieldNameStr+"values"+fieldValStr)
    cur.executemany("insert into "+tableNameToImport+ " "+fieldNameStr+"values"+fieldValStr,valLst)
    con.commit()
    #endregion
...
def __main():
    ...
    startTime = clock()
    #print(startTime)
    importShpBulk(importFilename, tableNameToImport)
    endTime = clock()
    #print(endTime)
