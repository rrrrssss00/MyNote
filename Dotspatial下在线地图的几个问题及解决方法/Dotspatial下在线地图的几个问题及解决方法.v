问题一：关闭图层后，再拖动，图层会自动再次出现，并自动打开
方法：
在WebMapLayer的UpdateStichedBasemap函数中，将
 if (map != null)
	 改为：
 if (map != null && _baseMapLayer.IsVisible)

问题二：当把某图层从地图控件中移除（使用map1.Layers.Remove或RemoveAt函数）时，不触发绑定的ItemRemove事件
方法：
